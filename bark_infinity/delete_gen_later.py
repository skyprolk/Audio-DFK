...
from bark_infinity.config import DEFAULTS, logger
from bark_infinity.config import VALID_HISTORY_PROMPT_DIRS, console
...
def generate_text_semantic_old(
    text,
    history_prompt=None,
    temp=0.7,
    top_k=None,
    ...
    """Generate semantic tokens from text."""
    if semantic_history_oversize_limit is not None:
        if semantic_history_oversize_limit > 512:
            semantic_history_oversize_limit = 512
        semantic_history_size_limit = semantic_history_oversize_limit
        encoded_text_size_limit = 512 - semantic_history_size_limit

    else:
        encoded_text_size_limit = 256
        semantic_history_size_limit = 256   
    ...
    if history_prompt is not None:
        ...
        semantic_history = previous_segment["semantic_prompt"]

    if history_prompt is not None or previous_segment is not None:
        assert (
            isinstance(semantic_history, np.ndarray)
            and len(semantic_history.shape) == 1
            and len(semantic_history) > 0
    ...
    tokenizer = model_container["tokenizer"]
    if text is None: 
        encoded_text = np.full(encoded_text_size_limit,TEXT_PAD_TOKEN)
    else:
        encoded_text = np.array(_tokenize(tokenizer, text)) + TEXT_ENCODING_OFFSET
    ...
    if OFFLOAD_CPU:
        model.to(models_devices["text"])
    device = next(model.parameters()).device
    if len(encoded_text) > encoded_text_size_limit:
        p = round((len(encoded_text) - encoded_text_size_limit) / len(encoded_text) * 100, 1)
        logger.warning(f"warning, text too long, lopping of last {p}%")
        encoded_text = encoded_text[:encoded_text_size_limit]

    encoded_text = np.pad(
        encoded_text,
        (0, encoded_text_size_limit - len(encoded_text)),
        constant_values=TEXT_PAD_TOKEN,
        mode="constant",
    )
    ...
    if semantic_history is not None:
        semantic_history = semantic_history.astype(np.int64)
        # lop off if history is too long, pad if needed
        semantic_history = semantic_history[-semantic_history_size_limit:]
        semantic_history = np.pad(
            semantic_history,
            (0, semantic_history_size_limit - len(semantic_history)),
            constant_values=SEMANTIC_PAD_TOKEN,
            mode="constant",
        )
    else:
        semantic_history = np.array([SEMANTIC_PAD_TOKEN] * semantic_history_size_limit)
    logging.debug(f"semantic_history len: {len(semantic_history)}")
    logging.debug(f"encoded_text len: {len(encoded_text)}")
    x = torch.from_numpy(
        np.hstack([
            encoded_text, semantic_history, np.array([SEMANTIC_INFER_TOKEN])
        ]).astype(np.int64)
    ...
    assert all(0 <= out) and all(out < SEMANTIC_VOCAB_SIZE)
    _clear_cuda_cache()
    token_probs_history = None
    return out, token_probs_history


def generate_coarse_old(
    x_semantic,
    history_prompt=None,
    temp=0.7,
    top_k=None,
    ...
    if history_prompt is not None:
        ...
        x_semantic_history = previous_segment["semantic_prompt"]
        x_coarse_history = previous_segment["coarse_prompt"]
...
def generate_fine_old(
    ...
    elif previous_segment is not None and previous_segment["fine_prompt"] is not None:

        x_fine_history = previous_segment["fine_prompt"]
...
def preload_models_old(
    ...
    print("These are models that are being loaded:")
    pprint(locals())
